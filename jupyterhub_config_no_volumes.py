...
c.OAuthenticator.oauth_callback_url = '{}/hub/oauth_callback'.format(
    os.environ['JUPYTERHUB_URL'])


class KeycloakMixin(OAuth2Mixin):
    # callback_url
    _OAUTH_AUTHORIZE_URL = '{}/auth'.format(
        os.environ['JUPYTERHUB_OAUTH_CLIENT_URL'])
    _OAUTH_ACCESS_TOKEN_URL = '{}/token'.format(
        os.environ['JUPYTERHUB_OAUTH_CLIENT_URL'])
